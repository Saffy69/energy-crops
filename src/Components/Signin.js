...
const reducer = (state, action) => {
  switch (action.type) {
    case "SEE":
      return { ...state, seen: true, content: "text" };
    case "UNSEE":
      return { ...state, seen: false, content: "password" };
    default:
      return state;
  }
};
            ...
            <form>
              <div className="mb-6 pr-4">
                <input
                  ...
                  type="email"
                  className="form-control block w-full  pl-4 py-2 text-xl font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none"
              ...
              <div className="flex justify-between items-center mb-6">
                ...
                <p
                  onClick={async () => {
                    try {
                      await forgotPassword(info.email.toString());
                  ...
                  className="ml-4 text-blue-600 cursor-pointer hover:text-blue-700 focus:text-blue-700 active:text-blue-800 duration-200 transition ease-in-out"
                >
                  Forgot password?
                </p>
              </div>
              <Link to="/">
                <button
                  onClick={async () => {
                    setLoading(true);
                    try {
                      await signIn(info.email, info.pass);
                    } catch (err) {
